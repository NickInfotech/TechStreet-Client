...
function Header() {
  ...
  return (
    <div className="w-full bg-whiteText md:sticky md:top-0 z-50">
      ...
      <div className="max-w-screen-xl mx-auto h-20 flex items-center justify-between px-4 lg:px-0">
        ...
        <div className="hidden md:inline-flex max-w-3xl w-full relative">
          <input
            ...
            placeholder: tracking-wide shadow-sm ring-1 ring-inset ring-gray-300
        ...
        {searchText && (
          <div
            className="absolute left-0 top-20 w-full mx-auto max-h-[500px] px-10 py-5 bg-white 
          z-20 overflow-y-scroll text-black shadow-lg shadow-[#d18657]"
          >
        ...
        <div className="flex items-center gap-x-6 text-2xl">
          <Link to={"/profile"}>
            {currentUser ? (
              <img src={currentUser?.avatar} alt="profileImage" className="w-8 h-8 rounded-full object-cover" />
            ) : (
              <FiUser className="hover:text-[#d18657] duration-200 cursor-pointer" />
            )}
          </Link>
          <Link to={"/favorite"} className="relative block">
            <FiStar className="hover:text-[#d18657] duration-200 cursor-pointer" />
          ...
          <Link to={"/cart"} className="relative block">
            <FiShoppingBag className="hover:text-[#d18657] duration-200 cursor-pointer" />
            <span className="inline-flex items-center justify-center bg-redText text-whiteText absolute -top-1 -right-2 text-[9px] rounded-full w-4 h-4">
      ...
      {/* Header Banner */}
      <div className="w-full bg-[#65442d] text-whiteText">
        <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
          <Menu>
            ...
            >
              <MenuItems
                anchor="bottom end"
                className="w-52 origin-top-right rounded-xl border border-white/5 bg-[#65442d] p-1 text-sm/6 text-gray-300 [--anchor-gap:var(--spacing-1)] focus:outline-none hover:text-white z-50"
              >
